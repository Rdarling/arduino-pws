...
// Activated by the magnet and reed switch in the rain gauge, attached to input D2
{
  ...
  if (raininterval > 10) // ignore switch-bounce glitches less than 10ms after initial edge
  {
    ...
    rainFall += 0.011;     // += 0.2794; //Increase amount of rain
...
void getSensors()
{
  ...
  float tempf = 0; // [temperature F] - RWD
  float pressure = 0;
  float mmHg = 0;
  float rainmm = 0; // rain mm from last report
  float rainin = 0; // rain inch from report - RWD
  ...
  rainin = rainFall;      // changed from mm to inch
  rainFall = 0;

  //Calc pressure
  pressure = myPressure.readPressure();   // pressure read from MLP in kPA
  pressure /= 100;                        // pressure divided by 100
